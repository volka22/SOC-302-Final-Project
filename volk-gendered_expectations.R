...
GSS <- mutate(GSS, no = ifelse(mawrkgrw == 2, 1, 0))
...
#model 1:
model1a <- glm(agree ~ man + woman + below_average + average + above_average + hs_or_less + associate + bach_or_more, data = my_dataset, family = binomial)
summary(model1a)

#model 2:
model2a <- glm(agree ~ often + sometimes + rarely + never ,  data = my_dataset, family = binomial)
summary(model2a)


#model 3:
model3a <- glm(agree ~ yes + no + agree_childsuffer + disagree_childsuffer, data = my_dataset, family = binomial)
summary(model3a)


#model 4 FINAL MODEL:
model4a <- glm(agree ~ yes + no +
                 often + sometimes + rarely + never +
                 below_average + average + above_average + 
